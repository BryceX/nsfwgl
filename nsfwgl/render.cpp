...
bool nsfw::RenderPass::setUniform(const char *name, nsfw::UNIFORM::TYPE type, const void *value, unsigned count, bool normalize) 
{
	...
	switch (type)
	{
	case nsfw::UNIFORM::FLO1: glUniform1fv(loc,1,(const GLfloat*)value);     break;
	case nsfw::UNIFORM::FLO3: glUniform3fv(loc, 1, (const GLfloat*)value);      break;
	case nsfw::UNIFORM::FLO4: glUniform4fv(loc, 1, (const GLfloat*)value);      break;
	...
	case nsfw::UNIFORM::TEX2: glActiveTexture(GL_TEXTURE0 + count); glBindTexture(GL_TEXTURE, *(const GLuint*)value); glUniform1i(loc, count);	break;
